...
- **Migration 0004 ÇÖZÜLDÜ (2026-07-16, aynı oturumda kapandı):** Kurucu bir şekilde prod'a bir yetki komutu çalıştırdı ama migration dosyasının kendisi değil, daha geniş bir komut — `service_role` hem `saves` HEM `profiles` üzerinde tam CRUD (SELECT+INSERT+UPDATE+DELETE) kazanmıştı, review'ın onayladığı "yalnız saves'e SELECT"ten çok daha geniş. Supabase MCP (proje `kmlogklnyxzptnrygyya`, "miras-oyunu") üzerinden tespit edilip düzeltildi: `0005_service_role_grant_correction` migration'ı hem prod'a `apply_migration` ile uygulandı (doğrulandı: artık tam onaylanmış minimal duruma dönük) hem yerel repoya commit+push edildi (`082a3e1`). Migration 0004 kendisi hâlâ Supabase'in migration history'sinde kayıtlı DEĞİL (muhtemelen `db push` değil ad-hoc komutla uygulandığı için) — zararsız, ileride biri `supabase db push` çalıştırırsa idempotent olarak kendini düzeltir.
- **CI durumu kontrol edilmedi** — son push'lardan sonra `gh run list --branch main` ile bakılabilir.
